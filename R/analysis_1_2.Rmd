...
We present avoided deforestation in hectares per year
...
new.dat = data.frame(cf_avd_mean = seq(from = min(cf_vcs_avd_comp_mean$cf_avd_min), 
                                       to = max(cf_vcs_avd_comp_mean$cf_avd_max, by = 1))) %>%
  mutate(VERRA = cf_avd_mean)

f1a <- cf_vcs_avd_comp_mean %>%
  # filter(n>=2) %>%
  ...
  ylab("Certified avoided deforestation (ha/yr)") + xlab("Quasi-experimental assessment of avoided deforestation (ha/yr)")+
  # theme_classic() +
...
Version of the above figure with points coloured by method.

```{r}

# read in method info from before

method_info <- read.csv(file.path(output_path, '/combined.csv')) %>% 
  mutate(ID = as.factor(ID))

# add VERRA info
  
df_for_method_plot <- method_info %>% 
  left_join(cf_vcs_comp_def %>% select(ID,VERRA), by = 'ID')

# plotting

df_for_method_plot %>% 
  ggplot(aes(y = VERRA, x = avd_def_yr, colour = method)) +
  geom_abline(intercept = 0, slope = 1, col = "black", linetype = 2, linewidth = 0.75)+
  geom_vline(xintercept = 0, col = "black", linetype = 1, linewidth = 0.5)+
  geom_hline(yintercept = 0, col = "black", linetype = 1, linewidth = 0.5)+
  ...
  scale_x_continuous(limits = c(-1000, 3500)) +
  scale_y_continuous(trans='log10', limits = c(50, 10000)) +
  ylab("Self-reported avoided deforestation (ha/yr)") + xlab("West et al assessment of avoided deforestation (ha/yr)")+
  theme(axis.line = element_blank(),
        axis.text = element_text(size = 12),
        legend.position="right",
        legend.spacing = unit(0, "cm")
...
f1b <- cf_vcs_avd_comp_mean %>%
  ...
  scale_color_manual(values = my_colors,
                    labels = c("1", "2","3","4")) +
  xlab("Certified avoided deforestation (ha/yr)") + ylab("Additionality ratio")+
...
ggsave(filename=file.path(fig_path,'Fig_1.png'),dpi=300,width=7,height=10)
...
#df %>% filter(ID %in% (cf_vcs_avd_comp_mean %>% filter(cf_Aadj_mean >1) %>% pull(ID))) %>% nrow() # aew edits - don't understand this
no_avd <- cf_vcs_avd_comp_mean %>% filter(cf_Aadj_mean >0) %>% nrow()
no_avd/nrow(cf_vcs_avd_comp_mean)
...
Scaling all the certified methods by the QEM:

```{r}

joint_subset_center_scale <- joint_subset %>% 
  group_by(ID) %>%
  mutate(
    mean_cf = mean(avd_def_yr[type == "cf"], na.rm = TRUE),
    sd_cf = sd(avd_def_yr[type == "cf"], na.rm = TRUE),
    scaled_value = (avd_def_yr - mean_cf) / sd_cf
  ) %>%
  ungroup()

joint_subset_center_scale

```

Also need to center and scale the certified value

```{r}

# extract values from above

mean_sd_table <- joint_subset_center_scale %>%
  filter(type == "cf") %>%
  select(ID, mean_cf, sd_cf) %>%
  mutate(ID=as.factor(ID)) %>% 
  distinct()

vcs_subset_center_scale <- vcs_subset %>%
  mutate(ID=as.factor(ID)) %>%
  left_join(mean_sd_table, by = "ID") %>%
  mutate(
    scaled_value = if_else(
      cf_source == "VERRA",
      (avd_def_yr - mean_cf) / sd_cf,
      NA_real_
    )
  )

vcs_subset_center_scale
...
joint_subset_center_scale %>%
  ...
  geom_hline(yintercept = 0) +
  # geom_boxplot(outlier.colour = NA) +
  #geom_boxplot(mapping=aes(y=scaled_value),position = position_dodge(width = 0.8)) +
  geom_point(aes(y = scaled_value), position = position_jitterdodge(0.6), alpha = 0.3, size = 3) +
#  geom_point(data = vcs_subset %>% filter(cf_source == 'VERRA'), aes(y = avd_def_yr, shape = cf_source), size = 3, alpha = 1, position = position_nudge(x = - 0.2)) +
  #scale_shape_manual(values = c(16,17), labels = c("vcs_official", "VERRA")) +
  scale_color_manual(values = c('red', 'blue'), labels = c("Certified", "QEM")) +
  #geom_point(data = vcs_subset_center_scale %>% filter(cf_source == 'VERRA'), aes(y = scaled_value), shape = 16, size = 3, alpha = 1, position = position_nudge(x = - 0.2)) +
geom_segment(data = vcs_subset_center_scale %>% filter(cf_source == 'VERRA'), 
             aes(x = as.numeric(ID) - 0.2, xend = as.numeric(ID) + 0.2, 
                 y = scaled_value, yend = scaled_value), 
             color = "red", size = 1, alpha = 0.8) +
  geom_hline(yintercept=3.09,colour='grey50',alpha=0.8,lty=2)+
  ylab('Scaled avoided deforestation (Z-score)') +
  xlab('Project')+
  theme(legend.title=element_blank())

ggsave(file.path(fig_path,'old_fig_2_aew.png'),
       bg = 'white',
       width = 6,
       height = 5,
       units = 'in'
...
Version with t-scores instead of Z-scores

```{r}

# add n column and t-score value

joint_subset_t <- joint_subset_center_scale %>% 
  group_by(ID) %>%
  mutate(n = sum(type == "cf"),
         t_score = (avd_def_yr - mean_cf) / (sd_cf / sqrt(n))) %>%
  ungroup()

joint_subset_t

mean_sd_table_t <- joint_subset_t %>%
  filter(type == "cf") %>%
  select(ID, mean_cf, sd_cf, n) %>%
  mutate(ID=as.factor(ID)) %>% 
  distinct()

vcs_subset_t <- vcs_subset %>%
  mutate(ID=as.factor(ID)) %>%
  left_join(mean_sd_table_t, by = "ID") %>%
  mutate(
    t_score = if_else(
      cf_source == "VERRA",
      (avd_def_yr - mean_cf) / (sd_cf / sqrt(n)),
      NA_real_
    )
  )
...
joint_subset_t %>%
  ...
  # geom_boxplot(outlier.colour = NA) +
  #geom_boxplot(mapping=aes(y=scaled_value),position = position_dodge(width = 0.8)) +
  geom_point(aes(y = t_score), position = position_jitterdodge(0.6), alpha = 0.3, size = 3) +
#  geom_point(data = vcs_subset %>% filter(cf_source == 'VERRA'), aes(y = avd_def_yr, shape = cf_source), size = 3, alpha = 1, position = position_nudge(x = - 0.2)) +
  #scale_shape_manual(values = c(16,17), labels = c("vcs_official", "VERRA")) +
  scale_color_manual(values = c('red', 'blue'), labels = c("Certified", "QEM")) +
  #geom_point(data = vcs_subset_center_scale %>% filter(cf_source == 'VERRA'), aes(y = scaled_value), shape = 16, size = 3, alpha = 1, position = position_nudge(x = - 0.2)) +
geom_segment(data = vcs_subset_t %>% filter(cf_source == 'VERRA'), 
             aes(x = as.numeric(ID) - 0.2, xend = as.numeric(ID) + 0.2, 
                 y = t_score, yend = t_score), 
             color = "red", size = 1, alpha = 0.8) +
  ylab('Scaled avoided deforestation (t-score)') +
  xlab('Project')+
  theme(legend.title=element_blank())

ggsave(file.path(fig_path,'old_fig_2_aew_t_scores.png'),
       bg = 'white',
       width = 6,
       height = 5,
       units = 'in'
...
dat_for_ttest <- joint_subset %>% 
  filter(type=='cf') %>% 
  select(ID,avd_def_yr)

vcs_for_ttest <- vcs_subset %>% 
  filter(cf_source=='VERRA') %>% 
  select(ID,avd_def_yr) %>% 
  mutate(t_value=NA,
         p_value=NA)

results <- vcs_for_ttest

for(i in 1:nrow(results)){
  
  project_no <- results[i,1]
  mu <- results[[i,2]]
  x <- dat_for_ttest %>% 
    filter(ID==project_no) %>% 
    pull(avd_def_yr) %>% 
    as.vector()
  ...
  results[i,3] <- result$statistic
  results[i,4] <- result$p.value
...
Repeating the stats for the different certification methods
