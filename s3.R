...
# plot s3a (scatter and line plot)
s3a_plot = cf_vcs_avd_comp_mean_df %>%
  ggplot(aes(y = VERRA, x = cf_avd_mean)) +
    geom_text_repel(aes(label=ID,color = as.factor(n)), size = 3, max.overlaps = 20)+
    ...
    scale_color_manual(values = my_colors,
                       labels = c("1", "2","3","4")) +
    ylab("Certified avoided deforestation (ha/year)") + xlab("Quasi-experimental assessment of avoided deforestation (ha/year)")+
    ...
    geom_point(data = neg_points_df, aes(x = cf_avd_mean, y = y_axis, color = as.factor(n)), shape = 1, 
               size=3) + # manually add VERRA negative points
    geom_text_repel(data = neg_points_df, aes(x = cf_avd_mean, y = y_axis, label = ID, color = as.factor(n)), 
                    size = 3, max.overlaps = 20) +  # add labels for neg points
...
s3b_plot = cf_vcs_avd_comp_mean_df %>%
  ggplot(aes(x = VERRA, y = cf_Aadj_mean, label = ID)) +
  ...
  scale_color_manual(values = my_colors,
                     labels = c("1", "2","3","4")) +
  xlab("Certified avoided deforestation (ha/year)") + ylab("Additionality ratio")+
  ...
  theme(axis.line = element_blank(),
        ...
        legend.spacing = unit(0, "cm")) +
             shape = 1, size = 3) +
  geom_text_repel(data = neg_points_df, aes(y = cf_Aadj_mean, x = 0, label = ID, color = as.factor(n)), 
                  size = 3, max.overlaps = 20) +  # add labels for neg points
  guides(shape = guide_legend(ncol=2, bycol=TRUE, title = NULL,
                              override.aes = list(size = 4, col = "black")),
         color = guide_legend(ncol=4, bycol=TRUE, title = "Number of studies", 
...
ggsave(s3_plot, filename=file.path("pngs",'s3_raw.png'),dpi=300,width=7,height=10)