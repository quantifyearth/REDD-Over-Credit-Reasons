...
all_projects_df$ID = as.factor(all_projects_df$ID)
...
neg_points_df = cf_vcs_avd_comp_mean_df %>%
  ...
  left_join(all_projects_df, by = c("ID"))
...
1/aadj_w_median
1/aadj_w_mean
# prepare data for plotting s8 (line data)
...
# plot s8 (scatter and line plot)
s8_plot = cf_vcs_avd_comp_mean_df %>%
  ...
  geom_point(size = 3, alpha = 0.8, aes(color = as.factor(country))) +
  scale_x_continuous(limits = c(-1000, 3500)) +
  scale_y_continuous(trans = "log10", limits = c(50, 10000)) +
  ylab("Certified avoided deforestation (ha/yr)") + 
  xlab("Quasi-experimental assessment of avoided deforestation (ha/yr)") +
  theme(axis.line = element_blank(),
        axis.text = element_text(size = 12),
        legend.position = "right",
        legend.spacing = unit(0, "cm")) +
  geom_point(data = neg_points_df, aes(x = cf_avd_mean, y = y_axis, color = as.factor(country)),
             shape = 1, size = 3) +
  guides(shape = guide_legend(ncol = 2, bycol = TRUE, title = NULL,
                              override.aes = list(size = 4, col = "black")),
         color = guide_legend(ncol = 1, bycol = TRUE, title = "Country:",
                              override.aes = list(shape = 16, size = 3, alpha = 0.5)))

ggsave(file.path("pngs", "s8_raw.png"), s8_plot, width = 8, height = 6, dpi = 300)
